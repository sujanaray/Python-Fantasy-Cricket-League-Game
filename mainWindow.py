...
##########################################################################################
    ...
    def file_save(self):
        ...
        for player in self.items:
            curs.execute("SELECT Points from player WHERE Player='"+player+"';")
        ...
        myteam= sqlite3.connect('match.db')
        curser= myteam.cursor()
        teamname= self.name.text()
        for i in range(len(List)):
            curser.execute("INSERT INTO team ('NAME','PLAYERS','SCORES') VALUES ('%s','%s','%d')"%(teamname,List[i][0],List[i][1]))
    ...
    def combo2(self):
        ...
        cu.execute("SELECT PlAYERS from team WHERE NAME='"+team+"';")
        player= cu.fetchall()
        ...
        cu.execute("SELECT SCORES from team WHERE NAME='"+team+"';")
        score=cu.fetchall()
        self.teamscore=[]
    ...
    def boxclose(self):
        ...
        myteam= sqlite3.connect('match.db')
        curser= myteam.cursor()
        curser.execute("SELECT PLAYERS from team WHERE NAME= '"+teamname+"';")
        hu= curser.fetchall()
    ...
    def load_names(self):
        ...
        Bowler= 'Bowler'
        sql    = "SELECT Player from player WHERE Type = '"+Batsman+"';"
        sql2   = "SELECT Player from player WHERE Type = '"+WicketKeeper+"';"
        sql3   = "SELECT Player from player WHERE Type ='"+Allrounder+"';"
        sql4   = "SELECT Player from player WHERE Type = '"+Bowler+"';"
